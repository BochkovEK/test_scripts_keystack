...
extract_charts() {
  ...
  log_info_block "Распаковываю чарты из каталога ./${archive_dir}"

  if ! find "$archive_dir" -maxdepth 1 -name "*.tgz" | grep -q .; then
    exit_on_error "Ошибка: в директории $archive_dir нет файлов .tgz"
  fi

  find "$archive_dir" -maxdepth 1 -name "*.tgz" | while read -r archive_file; do
    echo "Распаковываю чарт $archive_file"

    if ! tar -xzf "$archive_file" -C "$tmp_dir"; then
      exit_on_error "Ошибка извлечения $archive_file"
    fi
  done
}
...
helm_block_command() {
  if [ -n "$values_file" ]; then
    ...
    helm_command="helm upgrade $chart_name --install --create-namespace --namespace $chart_namespace --version $chart_version -f $values_file $tmp_dir/${chart_name}"
  else
    log_info_block "Устанавливаю чарт $chart_name"
    helm_command="helm upgrade $chart_name --install --create-namespace --namespace $chart_namespace --version $chart_version $tmp_dir/${chart_name}"
  fi
    ...
    if [[ $chart_install_flag == true ]]; then
      if [[ "$chart_order" == "$order_value" ]] && [[ $upgrade_flag == false ]]; then
        if [[ $chart_name == 'nexus3' ]]; then
          ...
          grafana_create_admin_password "$values_file"
        fi
        helm_block_command
      else
...
grafana_create_admin_password() {
  local grafana_yaml_file=$1
  local grafana_install_flag
  grafana_install_flag=$(yq '.charts[] | select(.name == "kube-prometheus-stack") | .install' "$config_file")
  local grafana_secret_namespace
  grafana_secret_namespace=$(yq '.charts[] | select(.name == "kube-prometheus-stack") | .namespace' "$config_file")
  local grafana_secret_name
  grafana_secret_name=$(yq '.grafana.admin.existingSecret' "$grafana_yaml_file")
  local grafana_user_key
  grafana_user_key=$(yq '.grafana.admin.userKey' "$grafana_yaml_file")
  local grafana_secret_key
  grafana_secret_key=$(yq '.grafana.admin.passwordKey' "$grafana_yaml_file")

  if [[ "${grafana_install_flag,,}" == "true" ]]; then
    if [[ "$grafana_secret_name" != "null" && "$grafana_secret_key" != "null" ]]; then
      create_secret_from_random_password \
        "$grafana_secret_name" \
        "admin" \
        "$grafana_secret_key" \
        "$grafana_user_key" \
        "$grafana_secret_namespace"
    fi
  fi
}
...
main() {
  ...
  extract_charts $charts_archive_dir
