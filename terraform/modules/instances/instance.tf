...
#  flavor_id                   = openstack_compute_flavor_v2.flavor[each.value].id
  ...
  dynamic "scheduler_hints" {
    ...
    content {
      group = each.value.server_group_type == "existing" ?
              data.openstack_compute_servergroup_v2.existing_group[each.value.server_group_name].id : openstack_compute_servergroup_v2.vm_group[each.value.base_name].id
    }
#    content {
