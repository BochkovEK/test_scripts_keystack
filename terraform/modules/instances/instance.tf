...
#  flavor_id                   = openstack_compute_flavor_v2.flavor[each.value].id
  key_pair                    = each.value.keypair_name == "" ? openstack_compute_keypair_v2.keypair.name : each.value.keypair_name
  security_groups             = each.value.security_groups == [] ? [openstack_compute_secgroup_v2.secgroup.name] : each.value.security_groups
